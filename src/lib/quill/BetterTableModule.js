...
import { matchTableCell, matchTableHeader, matchTable } from 'node_module/quil/utils/node-matchers'
    ...
    this.quill.root.addEventListener(
      ...
      (evt) => {
        ...
        const rowNode = path.filter((node) => {
          return (
            node.tagName && node.tagName.toUpperCase() === 'TR' && node.getAttribute('data-row')
          )
        })[0]

        const cellNode = path.filter((node) => {
          return (
            node.tagName && node.tagName.toUpperCase() === 'TD' && node.getAttribute('data-row')
          )
        })[0]
